...
KIOSKSERVICE
  ...
  # ── Allow www-data to restart the kiosk via the Settings UI ──
  info "Configuring sudoers so www-data can restart ${SERVICE_CLIENT}…"
  SUDOERS_FILE="/etc/sudoers.d/signage-refresh"
  cat > "$SUDOERS_FILE" << SUDOERSEOF
# Generated by signage install.sh — allows the web UI to trigger a kiosk restart
www-data ALL=(ALL) NOPASSWD: /bin/systemctl restart ${SERVICE_CLIENT}
www-data ALL=(ALL) NOPASSWD: /bin/systemctl restart ${SERVICE_CLIENT}.service
SUDOERSEOF
  chmod 0440 "$SUDOERS_FILE"
  success "sudoers entry written: ${SUDOERS_FILE}"
  info "Default refresh command: sudo systemctl restart ${SERVICE_CLIENT}"
  info "You can change it any time in the Settings UI → GERÄT / REFRESH"
